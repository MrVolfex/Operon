    ...
    <OwnerLayout>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
        <div style={{ background: 'var(--card)', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', padding: 20 }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 900 }}>{invoices.length}</div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['Invoice', 'Client', 'Work Order', 'Amount', 'Issued', 'Status'].map(h => (
                  <th key={h} style={{
            ...
            <tbody>
              ...
              {filtered.map((inv, i) => (
                <tr key={inv.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  <td style={{ padding: '13px 16px', fontSize: 12, fontWeight: 700, fontFamily: 'monospace' }}>
                    {inv.number ?? `#${inv.id}`}
                  </td>
                  ...
                  <td style={{ padding: '13px 16px', fontSize: 13, fontWeight: 800 }}>
